...
SELECT Count(m.id) -- 7997 
FROM   movie m; 

SELECT Count(g.movie_id) -- 14662 
FROM   genre g; 

SELECT Count(r.movie_id) -- 7997 
FROM   ratings r; 

SELECT Count(dm.movie_id) -- 3867 
FROM   director_mapping dm; 

SELECT Count(n.id) -- 25735 
FROM   names n; 

SELECT Count(rm.movie_id) -- 15615 
...
( 
           ...
           GROUP BY   languages ) 
SELECT   *, 
         sum(sum_votes) OVER w1 AS running_total 
FROM     german 
GROUP BY languages window w1 AS (ORDER BY languages rows UNBOUNDED PRECEDING)
ORDER BY running_total DESC;
-- Total number of votes for movies in German language = 1,06,79,617
-----------------------------------------------------------------------------
WITH italian AS 
( 
           SELECT     languages, 
           ...
           GROUP BY   languages ) 
SELECT   *, 
         sum(sum_votes) OVER w2 AS running_total 
FROM     italian 
GROUP BY languages window w2 AS (ORDER BY languages rows UNBOUNDED PRECEDING)
ORDER BY running_total DESC;
-------------------------------------------------------------------------------
-- Total number of votes for movies in Italian language = 64,61,605 
...
-- 'Drama', 'Comedy' and 'Thriller' are top 3 genres based on movie_count
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
SELECT * 
FROM   (SELECT genre, 
               year, 
               movie_name, 
               CASE 
                 WHEN Substr(worlwide_gross_income, 1, 1) = '$' THEN 
                 worlwide_gross_income 
                 ELSE Concat('$ ', Round(Cast(Trim(Substr(worlwide_gross_income, 
                                                   5, 
                                                   Length( 
                                                          worlwide_gross_income) 
                                                   )) 
               ...
               Dense_rank() 
                 OVER ( 
                   partition BY year 
                   ORDER BY year, worlwide_gross_income_in_usd DESC) AS 
               movie_rank 
        FROM   genre AS g, 
               (SELECT id, 
                       year, 
                       ...
                       CASE 
                         WHEN Substr(worlwide_gross_income, 1, 1) = '$' THEN 
                         Cast( 
                         Trim(Substr(worlwide_gross_income, 3, Length( 
                              worlwide_gross_income))) 
                         AS 
                         UNSIGNED 
                         ) 
                         ELSE Round(Cast(Trim(Substr(worlwide_gross_income, 5, 
                                              Length( 
                                              worlwide_gross_income))) 
                                                AS 
                                                      UNSIGNED) / 74.77) 
        ...
        WHERE  g.movie_id = m.id 
               AND g.genre IN (SELECT genre 
                               FROM   (SELECT genre, 
                                              Rank() 
                                                OVER ( 
                                                  ORDER BY Count(id) DESC) AS 
                                              rank_ 
                                       FROM   genre g, 
                                              movie m 
                                       WHERE  m.id = g.movie_id 
                                       GROUP  BY genre) AS top_grossing_movie 
                               WHERE  rank_ < 4)) AS mov 
WHERE  movie_rank < 6; 
...
( 
         ...
         ON       dm.name_id=n.id 
         JOIN     movie AS m 
         ON       dm.movie_id=m.id ), date_diff AS 
( 
       SELECT *, 
              Datediff(next_movie_date, date_published) AS diff 
       FROM   info_moviedate ), avg_inter_days AS 
( 
         SELECT   name_id, 
                  Avg(diff) AS avg_inter_movie_days 
         FROM     date_diff 
         GROUP BY name_id ), last_result AS 
( 
